          ...
          parse(Org.Paragraph,
                string("The $name data is classified with ",
                       ...
                       Dict(:all => "all $(size(data, 2))",
                            :sqrt => "\\(\\lfloor\\sqrt{$(size(data, 2))}\\,\\rceil = $(FC_mtry[config.mtry](size(data, 2)))\\)",
                            :log2 => "\\(\\lfloor \\log_2 $(size(data, 2)) \\rceil = $(FC_mtry[config.mtry](size(data, 2)))\\)",
                            :third => "\\(\\lfloor $(size(data, 2))/3 \\rceil = $(FC_mtry[config.mtry](size(data, 2)))\\)",
                            )[config.mtry],
